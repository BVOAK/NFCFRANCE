<?php
/**
 * Handlers Ajax pour le configurateur NFC - VERSION CORRIGÉE
 * Gère les appels JavaScript vers WordPress/WooCommerce
 */
...
/**
 * Handler pour ajout au panier avec configuration
 */
function nfc_add_to_cart_handler() {
    error_log('NFC: === DÉBUT AJOUT PANIER ===');
    ...
    try {
        ...
        error_log('NFC: Configuration décodée: ' . print_r($config, true));
...
function nfc_save_order_item_meta($item, $cart_item_key, $values, $order) {
    if (isset($values['nfc_config'])) {
        ...
        $item->add_meta_data('Couleur', ucfirst($config['color']));
        
        if (isset($config['user'])) {
            $item->add_meta_data('Nom sur la carte', $config['user']['firstName'] . ' ' . $config['user']['lastName']);
        }
        
        if (isset($config['image']) && !empty($config['image']['name'])) {
            $item->add_meta_data('Image personnalisée', $config['image']['name']);
            // Optionnel: sauvegarder l'image complète pour la production
        ...
        error_log('NFC: Métadonnées configuration sauvegardées dans la commande');
    }
}