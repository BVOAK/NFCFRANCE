...
if (in_array('woocommerce/woocommerce.php', apply_filters('active_plugins', get_option('active_plugins')))) {
    ...
    function gtmi_vcard_order_change_status($order_id, $old_status, $new_status)
    {
        ...
        if ('processing' === $new_status || 'completed' === $new_status) {
            error_log("GTMI_VCard: status of order $order_id changed from $old_status to $new_status");
            
            // ⭐ SEULE LIGNE AJOUTÉE - Appeler la fonction existante
    ...
    function gtmi_vcard_order_payment_success($order_id)
    {
        ...
        error_log("GTMI_VCard: Order $order_id successful");
        gtmi_vcard_new($order_id); // ✅ Cette ligne était déjà là
    }
